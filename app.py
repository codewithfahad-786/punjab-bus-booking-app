...
try:
    model = joblib.load("transport_fare_model.pkl")
    ...
    city_index = {
        city: i for i, city in enumerate(punjab_cities)
    }
...
if submit:
    ...
    if from_city == to_city:
        ...
        st.warning(
            "⚠️ From and To city cannot be same"
        )
        ...
        colB.metric(
            "Trip Cost",
            f"PKR {round(total_trip_cost,2)}"
        )


        colC.metric(
            "Final Price",
            f"PKR {round(final_price,2)}"
        )
